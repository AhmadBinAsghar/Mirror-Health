...
const SetContactNameModal = ({ visible, onClose, onSend, UserName }: Invite) => {
    ...
    return (
        ...
        >
            <View style={[styles.container, styles.boxShadow, { marginBottom: isKeyboardVisible ? 80 : 0 }]}>
                <LargeText text={en.Modals.Invite + " " + UserName + "?"} customStyle={{ fontSize: 20 }} />
                <SmallText text={en.Modals.setInviteeName} customStyle={{ marginTop: 6 }} />
                <AppInput value={name} placeholder={en.Inputs.setName} onChangeText={setName} containerStyle={{ width: wp(74), marginTop: 20, marginBottom: 0 }} customStyle={{ width: wp(60) }} />
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 }}>
                    ...
                    <AppButton disabled={disabled} text={en.Buttons.Confirm} onPress={() => { onSend && onSend(name), onClose() }} colors={disabled ? [AppColors.disabled, AppColors.disabled] : [AppColors.lightBlue, AppColors.darkBlue]} txtStyle={{ color: AppColors.white }} customStyle={{ borderWidth: 0, width: wp(36), height: 50 }} containerStyle={{ width: wp(36), height: 50 }} />
                </View>
            </View>
        </Modal>
    )
}
export default SetContactNameModal
